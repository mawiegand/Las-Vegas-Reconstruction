...
void kfusion::cuda::TsdfVolume::clearSlice(const kfusion::tsdf_buffer* buffer, const Vec3i offset) const
{
    device::Vec3i dims = device_cast<device::Vec3i>(dims_);
    device::Vec3f vsz  = device_cast<device::Vec3f>(getVoxelSize());
...
void kfusion::cuda::TsdfVolume::raycast(const Affine3f& camera_pose, tsdf_buffer& buffer, const Intr& intr, Depth& depth, Normals& normals)
{
    ...
    device::TsdfVolume volume(data_.ptr<ushort2>(), dims, vsz, trunc_dist_, max_weight_);
    device::raycast(volume, buffer, aff, Rinv, reproj, depth, n, raycast_step_factor_, gradient_delta_factor_);

}

void kfusion::cuda::TsdfVolume::raycast(const Affine3f& camera_pose, tsdf_buffer& buffer, const Intr& intr, Cloud& points, Normals& normals)
{
    ...
    device::TsdfVolume volume(data_.ptr<ushort2>(), dims, vsz, trunc_dist_, max_weight_);
    device::raycast(volume, buffer, aff, Rinv, reproj, p, n, raycast_step_factor_, gradient_delta_factor_);
}
                                               ...
                                               const Vec3i globalShift) const
{
    ...
    DeviceArray<float>& deviceData = (DeviceArray<float>&) data;
